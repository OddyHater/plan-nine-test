...
export const DonateConnection = ({ status, setStatus }: Props) => {
  ...
  return (
    <div className={'flex flex-col gap-6 max-w-[440px] mx-auto my-0'}>
      ...
      { status === 'enter' && <EnterDonationForm setData={setUserData} onSubmit={() => setStatus('active')} /> }
      { status === 'active' && <ActiveDonationForm onDecline={onDecline} onStepBack={onStepBack} phone={userData.phone} card={userData.card}/> }
    </div>
  );
};